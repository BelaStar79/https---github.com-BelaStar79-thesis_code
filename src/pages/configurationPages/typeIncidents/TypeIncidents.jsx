...
import { setIdRow, tipo_incidencia } from "../../components/others/table.js";

export default function TypeIncidents() {
  // editable variables
  const namePage = "Tipos de incidencia";
  const activeSection = "configuration";
  const activeOption = "Tipos de incidencia";
  const tableName = "Incidencias";
  const columnNames = ["Nombre", "Descripción", "Activo"];
  const columns = ["nombre", "descripcion", "activo"];
  ...
  const goToAddTypeIncidents = () => {
    navigate("/configuration/type-incidents/add/");
  };
  const goToEditTypeIncidents = (id) => {
    setIdRow(id - 1);
    navigate("/configuration/type-incidents/edit/");
  };
  ...
  const [totalPages, setTotalPages] = useState(
    Math.ceil(tipo_incidencia?.length / rowsPerPage)
  );
  ...
  const handleInputChange = (event) => {
    ...
    setTotalPages(Math.ceil(tipo_incidencia?.length / parseInt(value)));
  ...
  const filteredData = tipo_incidencia?.slice(startIndex, endIndex);
  if (isLoading) {
                    ...
                    <div className="formComponent__searchFormContainer">
                      <div className="formComponent__searchInputs">
                        <input
                          ...
                          placeholder="Nombre"
                          title="Introduzca el nombre de la incidencia que desea buscar"
                        />
                      </div>
                      <button
                            ...
                            <td className="tableComponent__tableContainer__td tableComponent__tableContainer__tdOptions">
                              <div className="tdOptions">
                                <button
                                  ...
                                  title="Editar"
                                  onClick={() => goToEditTypeIncidents(row.id)}
                                >
