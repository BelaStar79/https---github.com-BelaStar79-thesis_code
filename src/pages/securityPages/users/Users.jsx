...
import { setIdRow, usuarios } from "../../components/others/table.js";

export default function Users() {
  // editable variables
  const namePage = "Usuarios";
  const activeSection = "configuration";
  const activeOption = "Usuarios";
  const tableName = "Usuarios";
  const columnNames = ["Nombre", "Usuario", "Solapin", "Activo"];
  const columns = ["nombre", "user", "solapin", "activo"];
  ...
  const goToAddUsers = () => {
    navigate("/configuration/users/add/");
  };
  const goToDetailsUsers = (id) => {
    setIdRow(id - 1);
    navigate(`/configuration/users/details/`);
  };
  const goToEditUsers = (id) => {
    setIdRow(id - 1);
    navigate("/configuration/users/edit/");
  };
  ...
  const [totalPages, setTotalPages] = useState(
    Math.ceil(usuarios?.length / rowsPerPage)
  );
  ...
  const handleInputChange = (event) => {
    ...
    setTotalPages(Math.ceil(usuarios?.length / parseInt(value)));
  ...
  const filteredData = usuarios?.slice(startIndex, endIndex);
  if (isLoading) {
                    ...
                    <div className="formComponent__searchFormContainer">
                      <div className="formComponent__searchInputs">
                        <input
                          ...
                          placeholder="Nombre"
                          title="Introduzca el nombre del usuario que desea buscar"
                        />
                      </div>
                      <button
                    ...
                    <div className="formComponent__LabelInputs">
                      ...
                      <label htmlFor="" className="formComponent__label">
                        ...
                        <input
                          type="text"
                          className="formComponent__input"
                          placeholder="Introduzca el solapin"
                            ...
                            <td className="tableComponent__tableContainer__td tableComponent__tableContainer__tdOptions">
                              <div className="tdOptions">
                                <button
                                  ...
                                  title="Ver detalles"
                                  onClick={() => goToDetailsUsers(row.id)}
                                >
