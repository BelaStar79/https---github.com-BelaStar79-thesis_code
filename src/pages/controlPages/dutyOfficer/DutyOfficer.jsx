...
import { setIdRow, duty_officer } from "../../components/others/table.js";

export default function DutyOfficer() {
  // editable variables
  const namePage = "Asistencia";
  const activeSection = "control";
  const activeOption = "Oficial de guardia superior";
  const tableName = "Asistencia";
  const columnNames = [
    "Fecha",
    "Oficial de guardia",
  ...
  const columns = ["fecha", "nombre", "cantidad", "cantidadReal"];
  ...
  const goToAddDutyOfficer = () => {
    navigate("/control/duty-officer/add/");
  };
  const goToEditDutyOfficer = (id) => {
    setIdRow(id - 1);
    navigate("/control/duty-officer/edit/");
  };
  ...
  const [totalPages, setTotalPages] = useState(
    Math.ceil(duty_officer?.length / rowsPerPage)
  );
  ...
  const handleInputChange = (event) => {
    ...
    setTotalPages(Math.ceil(duty_officer?.length / parseInt(value)));
  ...
  const filteredData = duty_officer?.slice(startIndex, endIndex);
  if (isLoading) {
                    ...
                    <div className="formComponent__searchFormContainer">
                      <div className="formComponent__searchInputs">
                        <input
                          ...
                          placeholder="Nombre"
                          title="Introduzca el nombre del oficial que desea buscar"
                        />
                      </div>
                      <button
                            ...
                            <td className="tableComponent__tableContainer__td tableComponent__tableContainer__tdOptions">
                              <div className="tdOptions">
                                <button
                                  ...
                                  title="Editar"
                                  onClick={() => goToEditDutyOfficer(row.id)}
                                >
